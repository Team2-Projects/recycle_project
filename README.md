...
기본 경로는 4번 도착 직후부터 `5번 → HOME` 구간의 추론을 중지합니다.
중지 응답이 지연되어도 검출로 복귀 주행을 취소하지 않으며, 적재물이 있으면
HOME 도착 후 기존 하역 절차를 진행합니다. STOP·배터리 부족에 따른 HOME 복귀도
추론을 중지합니다. 하역 후 다시 순찰할 때는 ON 응답을 확인하고 재개합니다.
`navigation.launch.py`의 `home_return_start_index`(기본 `5`)는 수신 경로에서
추론을 끄기 시작할 목표의 **0부터 세는 순번**입니다. 선택 경로의 길이·순서를
바꾸면 이 값을 맞춰야 하며, 더 짧은 경로도 마지막 HOME 구간에서는 항상 중지합니다.
...
하역 요청 거절·실패 시에는 ON을 요청하고, STOP·배터리 부족으로 취소하면
HOME 복귀 중 OFF를 유지합니다. 통신 실패는 경고와 함께 재시도하며,
